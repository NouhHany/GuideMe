...
class _RegistrationScreenState extends State<RegistrationScreen> {
  ...
  Future<void> _register() async {
    ...
    if (!_validatePassword(password)) {
      setState(() {
        _passwordError =
            'Password must be at least 6 characters containing: UPPERCASE, lowercase, Number, and (*!@#).';
      });
      return;
    }
    ...
    try {
      ...
      switch (e.code) {
        ...
        case 'invalid-email':
          errorMessage =
              'The email format is not correct. Please check and try again.';
          break;
        case 'weak-password':
          errorMessage =
  ...
  Future<void> _handleGoogleSignIn() async {
    try {
      ...
      try {
        ...
        if (e.code == 'account-exists-with-different-credential') {
          ...
          if (signInMethods.contains('password')) {
            _showMessageOverlay(
              'This email is linked to an account. Please sign in with your email and password.',
            );
      ...
      if (!mounted) return;
      _showMessageOverlay(
        'An issue occurred with Google Sign-In. Please try again.',
      );
    }
  }
